...
!----------------------------------------------------------
  function SpMtx_AB(A,B,AT,BT) result(AB)
    ...
    if (present(AT).and.AT) then
      ...
      Anrows=A%Ncols
      Ancols=A%Nrows
      call SpMtx_arrange(A,D_SpMtx_ARRNG_COLS,sort=.false.)        
    else
      Aindi=>A%indi
      Aindj=>A%indj
      Anrows=A%Nrows
    ...
    if (present(BT).and.BT) then
      ...
      Bnrows=B%Ncols
      Bncols=B%Nrows
      call SpMtx_arrange(B,D_SpMtx_ARRNG_COLS,sort=.false.)
    else
      Bindi=>B%indi
      Bindj=>B%indj
      Bnrows=B%Nrows
      Bncols=B%Ncols
      call SpMtx_arrange(B,D_SpMtx_ARRNG_ROWS,sort=.false.)        
    endif
    if (Ancols /= Bnrows) then
      print*, "ERROR: A*B is impossible!!!"
      print *,'Ancols=',Ancols,'Bnrows=',Bnrows
      stop
    endif
  ...
  try:do i=1,Anrows
    ...
    AB = SpMtx_newInit(nnz=nnz,             &
           ...
           arrange_type=D_SpMtx_ARRNG_ROWS, &
                M_bound=ABb)
!print *,'AB:',AB%nrows,AB%ncols,AB%nnz
    deallocate(ABv)
    deallocate(ABj)
    deallocate(ABi)

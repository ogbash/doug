...
AC_DEFUN([GET_SIZEOF_FORTRAN_TYPE],[
...
LOG_COMMAND([$CC $CFLAGS -I. -c conftest.c],
    LOG_COMMAND([$FC $FFLAGS conftestf.f conftest.o -o conftest $LDFLAGS $LIBS],
	LOG_COMMAND([./conftest],[HAPPY=1],[HAPPY=0]),
	[HAPPY=0]),
    [HAPPY=0])
...
if test "$HAPPY" = "1" -a -f conftestval; then
    ...
    LOG_MSG([here is the C program:], 1)
    LOG_FILE([conftest.c])
    if test -f conftest.h; then
	LOG_MSG([here is contest.h:], 1)
	LOG_FILE([conftest.h])
    fi
    LOG_MSG([here is the fortran program:], 1)

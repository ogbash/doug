...
!! With no overlap each mesh node belongs only to one process region. So called \em interface (\em ghost values) for region 1 is shown on the picture below.
!! \image html ghosts.png "Non-overlapping regions with ghost nodes outside the region"
!!
...
!! Overlap is constructed by adding connected nodes in the mesh graph one \em layer at a time. The overlap of 2 (ol=2) means there will be 4 layers on the boundary that are shared by both processes. However, the ghost values are now in the last layer \b inside the region, not outside, so with ol=1 the ghost values are the same as in non-overlapping case. This is a trick which allows to reduce the amount of calculation in the overlapping case.
!! \image html ghosts_ol.png "Overlapping regions with ghost nodes in the last layer of the region"
!!  
!! <em>The general assumption is that the region values are up-to-date before and after each operation.</em>
!! - The parallel mv multiplication is done by SpMtx_operation::SpMtx_pmvm_assembled subroutine:
!!  -# full multiplication for the region except the ghost values (which are used but not written) is done
!!  -# new ghost values are received from neighbour processes, so our general assumption holds
!! - The local solves are done for the whole region with the combination step where corresponding values are added in case of Additive Schwarz method.
!!
...
!! -# local to local (2,2)
!! -# ghost to local (from outside the region in non-overlapping case)
!! -# A_ghost (ghost to ghost inside the region in overlapping case)
!!
...
!!
!! The vector indices that need to be sent and received during matrix-vector multiplication are stored in \c ax_sendidx and \c ax_recvidx members of the Mesh_class::Mesh class.
!!  The vector indices that need to be exchanged (sent/received) during first level preconditioner are stored in \c ol_solve array.

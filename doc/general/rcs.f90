...
!!
!! \section input Input
!!  - \f$ R_i \f$ - injection matrices mapping from the vectors taking 
!!       values at all points of the fine mesh to vectors taking values at the 
!!       points in the interior of the support of basis function i. This also identifies aggregates 
...
!!
!! To get restriction matrix we need to solve the problem (\e g - vector of Lagrange multipliers in the energy minimization problem)
...
!!
!! Multiplying a vector by the matrix C can be implemented efficiently 
!!   using the following formulas which involve only sparse matrices
!! \f[
!!   B_j^{-1} = A_j - A_j
